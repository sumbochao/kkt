   ...
   function getNotifyToSend()
    {
        ...
        $sql = "SELECT * FROM g_notice_queue Where os_type = 2 Order by id DESC LIMIT 200";
        var_dump($sql) ;
   ...
   function send_push_notification($messageId,$registatoin_ids, $message,$channel) {
        ...
        try {
            ...
            if ($arrResult->failure === 1) {
                 ...
                 if(strcasecmp("MissingRegistration",$arrError[0]->error)==0||strcasecmp("InvalidRegistration",$arrError[0]->error)==0||strcasecmp("NotRegistered",$arrError[0]->error)==0||strcasecmp("MismatchSenderId",$arrError[0]->error)==0){
