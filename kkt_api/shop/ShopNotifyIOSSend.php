...
try
{
    ...
    require_once('APNSBase.php');
    require_once('APFeedback.php');
...
{
    global $config;
    if ($fp = fopen($config['logSendIOS'], 'at'))
    {
   ...
   function getNotifyToSend()
    {
        ...
        $sql = "SELECT * FROM ms_notify_queue Where os_type = 3  LIMIT 200";
    ...
    {   
        try{
            $connect = $this->connectDB();
            $sql = "Delete From ms_notify_queue  WHERE id = ?";
            # creating the statement
            $q = $connect->prepare($sql);
            $arrV = array($id);
    ...
    function sendNotification($messageId, $deviceToken, $payload)
    {
       ...
       $feedback->setPrivateKey($this->certificate); 
       $feedback->setPrivateKeyPassphrase($this->passphrase);
       $fb=$feedback->receive();
       foreach($fb as $itemFB){
           writeToLog('FB Token:-->'.$itemFB["devtoken"]);
       }
       var_dump($fb);

        writeToLog('Message successfully delivered:-->'.$result);
        return TRUE;
    }
    ...
    writeToLog('Message successfully delivered:-->loi'.$error_response['status_code']);
