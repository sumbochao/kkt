...
{
    global $config;
    if ($fp = fopen($config['logSendAndroid'], 'at'))
    {
...
class APNS_Android_Push
{
   ...
   function getNotifyToSend()
    {
        ...
        $sql = "SELECT * FROM ms_notify_queue Where os_type = 2  LIMIT 200";
   ...
   function deleteToken($token)
    {   
        try{
            $connect = $this->connectDB();
            $sql = "Delete From ms_notify_user  WHERE device_token = ?";
            # creating the statement
            $q = $connect->prepare($sql);
            $arrV = array($token);
            $count = $q->execute($arrV);
   ...
   function send_push_notification($messageId,$registatoin_ids, $message) {
        // Set POST variables
        $channel =  0;
        $arrChannelKey = Array("0"=>"AIzaSyD6k1o9kvkwy0oNGZ5E-uQiNl_cQWPemc8");
        try {
            ...
            if ($arrResult->failure === 1) {
                 ...
                 if(strcasecmp("MissingRegistration",$arrError[0]->error)==0||strcasecmp("InvalidRegistration",$arrError[0]->error)==0||strcasecmp("NotRegistered",$arrError[0]->error)==0){
