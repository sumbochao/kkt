  ...
  try
{
   ...
   // writeToLog("Notify_Push_Queue script started ($mode mode)");
    ...
    $obj = new Audio_Notify_Push_Queue($config);
...
{
    global $config;
    if ($fp = fopen($config['logfileQueue'], 'at'))
    {
                   ...
                   if (empty($to_user))
                    {
                       ...
                       foreach($arr_token as $token){
                           echo "--------->app_client_id: ".$token["app_client_id"]."-".$message["audio_id"]."\n";
                            
                           $payload = $this->createPayLoadNotify($message["content"],"view",0,$message["audio_id"],$message["from_user"],$icon);
  ...
  function insertNotifyToQueue($notice_id,$device_token,$payload,$time_sent,$os_type)
    {   
        ...
        try{
            ...
            $sql = "INSERT INTO c_story_audio_notice_queue (notice_id,device_token,payload,time_queued,time_sent, os_type)
    ...
    {   
        try{
            $connect = $this->connectDB();
            $sql = "UPDATE c_story_audio_notice SET STATUS = 0 WHERE id = ? ";
    ...
    {
        $connect = $this->connectDB();
        $sql = "SELECT id,audio_id,from_user,to_user,content,url,icon,STATUS,time_sent,create_date,create_user FROM c_story_audio_notice WHERE STATUS = 1 LIMIT 0, 100;";
     //  echo $sql;
            # creating the statement
            $q = $connect->prepare($sql);
            $q->execute(array());   
    ...
    {
        $connect = $this->connectDB();
        $sql = "SELECT  DISTINCT app_client_id,user_id,device_token,os_type FROM c_story_audio_notice_user ";
    //   echo $sql;
            # creating the statement
            $q = $connect->prepare($sql);
            $q->execute(array());   
    ...
    function getTokenByUserIds($ids)
    {
        ...
        $sql = "SELECT user_id,device_token,os_type FROM c_story_audio_notice_user Where user_id in (".$ids.") ";
