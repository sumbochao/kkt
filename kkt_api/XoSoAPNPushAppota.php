...
try
{
    ...
    /*if ($argc != 2 || ($argv[1] != 'development' && $argv[1] != 'production'))
        exit("Usage: php XoSoAPNPush.php development|production". PHP_EOL);
        */
  //  $mode = $argv[1];
    $mode="appota";
    $config = $config[$mode];
       ...
       while (true)
        {
            ...
            $stmt = $this->pdo->prepare('SELECT * FROM xs_notice_ios_queue where app_header = "XOSO-APPOTA-IOS" LIMIT 20');
            $stmt->execute();
            ...
            foreach ($messages as $message)
            {
               if ($this->sendNotification($message->id, $message->device_token, $message->payload))
                {
                   var_dump($message->id);
                    $stmt = $this->pdo->prepare('Delete From xs_notice_ios_queue  WHERE id = ?');
