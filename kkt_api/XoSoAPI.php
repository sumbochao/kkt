    ...
    switch($action){
       ...
       case "getThreadHotComment": {
        ...
        case "postComment": { 
            ...
            $app_client_id = isset($_POST['id_user'])?$_POST['id_user'] :"";  
           ...
           */
            if(empty($id_thread)||empty($app_client_id)||empty($comment)||empty($username)){
                 $arrOut = array();
                 $arrOut["result"]=1;
                 echo json_encode($arrOut);   
                 return;
            ...
            $id_user = getIdUserFromClientId($app_client_id);
            if($id_user==0){
                 $arrOut = array();
                 $arrOut["result"]=3;
                 echo json_encode($arrOut);   
       ...
       case "getTKDauSoMoThuong": {
        ...
        case "getTKGiaiDacBietTheoNgay": {
         ...
         case "getAppHeaderXoSo": {
              ...
              if(empty($app_header)) echo ""; 
                  ...
                  $arrOut = getAppHeaderXoSo($app_header,$app_client_id,$os_version,$ip_address,$imei); 
                  echo json_encode($arrOut);   
              }
        ...
        case "updateUserName": {
         ...
         case "createPasswordWeb": {
            ...
            if(empty($password)||empty($app_client_id)) {echo 1;return;}
         
            $arrOut = createPasswordWeb($app_client_id,$password); 
            if(!$arrOut) {echo 3;return;} else echo 0;
        ...
        case "updateUser": {
            ...
            if(checkUserNameUpdateInfo($username,$app_client_id)==1) {echo 2;return;}
            if(!(empty($mobile)||$mobile=="")){
                if(checkMobileUpdateInfo($mobile,$app_client_id)==1) {echo 5;return;}
            }else{
              $mobile = null;  
            }
         ...
         case "addKenByAppClient": {
            ...
            if(empty($app_client_id)) {echo 1;return;}
            $result = addKenByAppClient($app_client_id,$ken); 
        ...
        case "insertChat": {
            ...
            $chatContent = "app_client_id:".$app_client_id." content:".$content." deviceName:".$deviceName." region:".$region;
          //  echo $chatContent;
          //  writeLog($chatContent);
            ...
            $result = insertChat($id_user,$username,$avatar_url,$content,$deviceName,$region);
         ...
         case "registryNoticeIOS": { 
              ...
              if(empty($device_token)||empty($ip_address)||strcmp("(null)",$device_token)==0) {echo 1;return;}
              
             // if(checkTokenIOSExist($app_client_id)>0) updateTokenIOSNotice($app_client_id,$device_token); 
               insertNoticeIOSUser(intval($app_client_id),$device_token,$username,$ip_address,$app_header);
        ...
        case "getNoticeAdroid": { 
              ...
              if(empty($app_client_id)) {return;}
              
              $notice = getNoticeAdroid($app_client_id);
              if(!empty($notice)) {
                  echo json_encode($notice);
                  deleteNoticeAndroid($notice["id"]);
              }
        ...
        default:  {
            ...
            echo "<h2 style='color:red'>getThreadHotCommentFromId:</h2>";
            echo "<em>Lấy  Ds comment từ Id.</em><br/>";
            echo "<ul>Tham Số:";
            echo "<li>action: getThreadHotCommentFromId</li>"; 
            echo "<li>id_thread: id chủ đề hot</li>";  
            echo "<li>id: id comment </li>";  
            echo "<li>limit: số item lấy</li>";   
            echo "</ul>";
            echo "<span>Ví dụ: <a href='http://kenhkiemtien.com/kkt_api/XoSoAPI.php?action=getThreadHotCommentFromId&id_thread=1&id=1&limit=10' target='_blank'> Danh sách comment từ Id</a></span>" ;
            ...
            echo "</ul>";
            echo "<span>Ví dụ: <a href='http://kenhkiemtien.com/kkt_api/XoSoAPI.php?action=getAppHeaderXoSo&app_header=TUVI-APPOTA-APK&app_client_id=123&imei=124' target='_blank'>Lấy thông số cầu hình của app.</a></span>" ;     
            ...
            echo "</ul>";
            echo "<span>Ví dụ: <a href='http://kenhkiemtien.com/kkt_api/XoSoAPI.php?action=createPasswordWeb&app_client_id=123&password=thangtt' target='_blank'>Cap nhat ten tai khoan.</a></span>" ; 
            ...
            echo "</ul>";
            echo "<span>Ví dụ: <a href='http://kenhkiemtien.com/kkt_api/XoSoAPI.php?action=registryNoticeIOS&app_client_id=123&device_token=123token&ip_address=123.456' target='_blank'>Đăng ký token send notice cho ios</a></span>" ;
            ...
            echo "<li>app_client_id: user_id cấp cho app</li>";  
            echo "</ul>";
            echo "<span>Ví dụ: <a href='http://kenhkiemtien.com/kkt_api/XoSoAPI.php?action=getNoticeAdroid&app_client_id=123' target='_blank'>Get notice cho Android</a></span>" ;
