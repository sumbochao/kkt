<?php
    ...
    if(isset($_GET["catid"]) && isset($_GET["p"])){        
        ...
        $video = GetAllVideo($categoryId, $currentPage, $numberRecordPerPage);
        $totalVideo = GetTotalVideo($categoryId);
        $totalPage = ceil($totalVideo / $numberRecordPerPage);
                
        foreach($video as $i=>$row){
            $urlImg = getImage($row["picture"], "video", $row["create_date"], "m");
            $item[$i]["id"] = $row["id"]; 
        ...
        $data["action"]["url"] = "/GetVideoList.php?catid=" . $categoryId . "&p=";
        $data["item"] = $item;
    } else {
        ...
        $data["totalPage"] = "0";
        $data["action"]["type"] = "aURL";
        $data["action"]["url"] = "/GetVideoList.php?catid=&p=";
        $data["item"] = array();        
    }
