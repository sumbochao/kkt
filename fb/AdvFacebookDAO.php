...
";
    ...
    function getCampaignInvite($account_id)
    {
        
        $sql = "SELECT id,app_id,title,count_sent,create_date FROM vtc_adv_facebook.campaign_invited Where account_id = '".$account_id."' ORDER By  id DESC"; 
    ...
    function getFacebookUserIdInvitable($account_id,$campaign_id)
    {
        ...
        $sql = "SELECT  facebook_user_id FROM vtc_adv_facebook.facebook_user_invittable WHERE account_id = ? AND facebook_user_id NOT IN (SELECT facebook_user_id FROM campaign_invited_log WHERE campaign_id = ?) LIMIT  30;
"; 
        # creating the statement
        $connect = connectAdvFacebookDB();
