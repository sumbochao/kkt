...
ECHO.
ECHO. 
ECHO (0) Apktool Dissasembly (src apk_in - dest project)
ECHO (1) Apktool Assembly (src project - dest project)
ECHO (2) DISASSEMBLY a classes.dex with smali
ECHO (3) ASSEMBLY a classes.dex with baksmali
ECHO (4) Sign and ZipAlign an APK
ECHO (5) Convert an Xml Binairy to Xml
ECHO (6) Launch CMD
ECHO (7) Launch Notepad++
ECHO (8) About
ECHO (9) EXIT
ECHO (10) Resign an apk
ECHO.
set INPUT=
...
java -jar SYSTEM.EAD/apktool.jar d apk_in/%FICHIER% project/dis_%FICHIER%
goto end

:apktoolcompile
set /P FICHIER=Enter the name of the folder to assembly (ex: dis_Name.apk) : %=%
java -jar SYSTEM.EAD/apktool.jar b project/%FICHIER%
echo I: %FICHIER%/dist/%FICHIER:~4% %FICHIER%/dist/signed_%FICHIER:~4%
