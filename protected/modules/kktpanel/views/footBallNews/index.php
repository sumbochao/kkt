...
<script type="text/javascript">
    function ajaxDelete(id){
        var strUrl = "<?=$url->createUrl("footBallNews/ajaxDeleteNews") ?>";
    ...
    function ajaxQuickUpdate(id){
        var strUrl = "<?=$url->createUrl("footBallNews/ajaxQuickUpdate") ?>";
        var status = 0;
    ...
    <div class="box">
        <div class="fillter clearfix">
            <div class="fl">
                ...
                <a href="<?php echo $url->createUrl("footBallNews/create")?>"><input type="button" class="btn-bigblue" value=" Thêm mới "></a>
            </div>
            <div class="fr">
                <ul class="paging">
                    <?php
                ...
                <?php foreach($data as $key=>$value){$key+=1;?>
                    <tr>
                        ...
                        <td>
                            ...
                            <br>
                            <img style="width: 50%;" src="<?php echo Yii::app()->params['urlImages']?>bongda/news/<?php echo  date('Y/md', strtotime($value['create_date']))?>/<?php echo $value['image']?>" />
                        ...
                        <td>
                            ...
                            <a href="<?php echo $url->createUrl("footBallNews/edit",array("id"=>$value["id"]))?>"> Sửa </a> | 
...
<script type="text/javascript">
	...
	{
		var strUrl = "<?=$url->createUrl("footBallNews/ajaxGetNotify") ?>";
		$.ajax({
			type: 'POST',
			url: strUrl,
			data:{
	...
	{
		var strUrl = "<?=$url->createUrl("footBallNews/ajaxDeleteNotify") ?>";
		if(confirm('Bạn có chắc chắn muốn xóa không ?')){
			$.ajax({
				type: 'POST',
				url: strUrl,
	...
	{
		var strUrl = "<?=$url->createUrl("footBallNews/ajaxSaveNotify") ?>";
