...
<script type="text/javascript">
    ...
    function runExcel(email){
        ...
        $(document).ready(function() {
            $('#chkAll').click(function(event) {  //on click 
                if(this.checked) { // check select status
                    $("input[name='chkItem[]']").each(function() { //loop through each checkbox
        ...
        var userIdlist ="";
         $(document).ready(function() {
            $('#chonApp').click(function(event) {  //on click 
                ...
                if(userIdlist==""){
                    alert("Bạn phải chọn it nhất 1 tài khoản");
                }else
                 ajaxAddUserToApp(userIdlist,$( "#appList" ).val());
            });
            
        });
        
        
   function ajaxAddUserToApp(userIdlist,appId){
        var strUrl = "<?=$url->createUrl("fbaccount/ajaxAddUserToApp") ?>";
            $.ajax({
                type: "POST",
                url: strUrl,
                data: {
                ...
                <li class="clearfix"><label>&nbsp;</label>
                    <div class="filltext">
                        <?php if($user == 0){?>
                            ...
                            <a href="<?php echo $login_add;?>">
                                <input type="button" class="btn-bigblue" value=" Đăng nhập và lấy bạn ">
                            </a>
                            <?php }else{?>
    ...
    <div class="box">
        <div class="fillter clearfix">
            <div class="fl">
                ...
                <a href="<?php echo $url->createUrl("fbaccount/create")?>"><input type="button" class="btn-bigblue" value=" Thêm mới "></a> <a id="login" href="javascript:void(0)" onclick='$("#appdialog").dialog("open"); return false;'> 
                                <input type="button" class="btn-bigblue" value=" Chon App ">
                             </a> 
            ...
            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                <tbody>
                    <tr class="bg-grey">
                        ...
                        <td><strong><input type="checkbox" name="chkAll" id="chkAll" /></strong></td>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><input type="checkbox" name="chkItem[]" id="chkItem" value="<?php echo $value["id"];?>" /></td>
                            <td style="background-color :<?php echo $color;?> ;"><a  href="<?php echo empty($value["link"])?"javascript:void(1)":$value["link"]?>" target="_blank"><?php echo $value["username"]?></a></td>
                            <td><?php echo $value["password"]?></td>
                            <td><?php echo $value["birthday"]?><br/><?php echo $value["mobile"]?></td>
                            <td>
							<a href="<?php echo $url->createUrl("fbaccount/friendList",array("id"=>$value["id"]))?>" target="_blank">
                                <?php 
                            ...
                            <td>
                                ...
                                <?php if($user_email == $value["username"]){?>
                                    <a href="<?php echo $url->createUrl("fbfriend/index",array("id"=>$value["id"]))?>"> DS Kết bạn </a> -
                    ...
                    <tbody>
                        ...
                        <?php foreach($invite_friend as $value){ ?>
                            <tr>
                                ...
                                <td>
                                <?php  $fids = explode("_", $value["picture"]["data"]["url"]); echo $fids[1] ?>
                                </td>
                                <td>
                                    <?php echo $value['id'];?>
                                </td>
...
?>
 ...
 <?php
    ...
    <div class="main clearfix">
        ...
        <input type="button" value="Chon APP" id="chonApp" class="btn-bigblue" name="chonApp"/>
           <?php
            }
        ?>
