...
<script type="text/javascript">
    function ajaxQuickUpdate(id){
        ...
        var strUrl = "<?=$url->createUrl("comic/ajaxQuickUpdate") ?>";
        $.ajax({
            type: "POST",
            url: strUrl,
            data: {
    ...
    function ajaxDelete(id){
        var strUrl = "<?=$url->createUrl("comic/ajaxDelete") ?>";
    ...
    <div class="box">
        <div class="fillter clearfix">
            <div class="fl">
                ...
                <a href="<?php echo $url->createUrl("comic/create")?>"><input type="button" class="btn-bigblue" value=" Thêm mới "></a>
            </div>
            <div class="fr">
                <ul class="paging">
                    <?php
            ...
            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                <tbody>
                    <tr class="bg-grey">
                        ...
                        <td width="8%"><strong>Tác giả</strong></td>
                        <td width="5%"><strong>Get Link</strong></td>
                        <td width="7%"><strong>Tình trạng</strong></td>
                        <td width="7%"><strong>Trạng thái</strong></td>
                    ...
                    <?php foreach($data as $key=>$value){$key+=1;?>
                        <tr>
                            ...
                            <td><img style="width: 75px; height: 75px;" src="<?php echo Yii::app()->params["base_url"].$value['image']?>" /></td>
                            <td>
                                <?php 
                                    $x="";
                                    foreach(AComic::getCheckCategory($value['id']) as $key1 =>$result){
                                         $x= $x.",".$data_cat[$result];
                                    }
                                    echo ltrim($x, ",");
                            ...
                            <td><?php if ($value["get_link_by"]==1){echo "Link Server TTV";}else{echo "Link nguồn";}?></td> 
                            <td><?php if ($value["status"]==1){echo "Hoàn thành";}else{echo "Chưa hoàn thành";}?></td> 
                            <td><input type="checkbox" value="1" id="status_<?php echo $value["id"]?>" <?php echo $value["status_view"]==1 ? 'checked="checked"':'';?>> Hiển thị</td>
                            ...
                            <td>
                                ...
                                <a href="<?php echo $url->createUrl("comic/edit",array("id"=>$value["id"]))?>"> Sửa </a> | 
