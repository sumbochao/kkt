...
<script type="text/javascript">
    function ajaxQuickUpdate(id){
        ...
        var strUrl = "<?=$url->createUrl("app/ajaxQuickUpdate") ?>";
        $.ajax({
            type: "POST",
            url: strUrl,
            data: {
    ...
    function ajaxDelete(id){
        var strUrl = "<?=$url->createUrl("app/ajaxDelete") ?>";
    ...
    <div class="box">
        <div class="fillter clearfix">
            <div class="fl">
                ...
                <a href="<?php echo $url->createUrl("app/create")?>"><input type="button" class="btn-bigblue" value=" Thêm mới "></a>
            </div>
            <div class="fr">
                <ul class="paging">
                    <?php
                    ...
                    <?php foreach($data as $key=>$value){?>
                        <tr>
                            ...
                            <td>
                                <?php 
                                    if(!empty($value['picture'])){
                                        echo '<img style="width: 100px; height: 100px;" src="'.Common::getImage($value['picture'],"app",$value['create_date'],0).'"';
                                    }
                                ?>
                            </td>
                            ...
                            <a class="s14" href="javascript:void(0)" onclick="ajaxQuickUpdate('<?php echo $value["id"]?>')"> Lưu </a> | 
                                <a href="<?php echo $url->createUrl("app/edit",array("id"=>$value["id"]))?>"> Sửa </a> | 
