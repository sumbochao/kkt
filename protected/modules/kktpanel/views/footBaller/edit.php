...
<script type="text/javascript">
    window.onload = function() {
        var configUploadData = {
            ...
            create_date: "<?php echo strtotime($footballer['create_date']); ?>",        
    ...
    {
        var strUrl = "<?=$url->createUrl("footBaller/ajaxUpdate") ?>";
        var name = $('#name').val();
        ...
        $.ajax({
            ...
            success: function(msg){
                if(msg == 1){
                    ...
                    window.location = '<?php echo $url->createUrl("footBaller/index",array("page"=>$page))?>';
                }else{
                    $("#show_error").html(msg);
                }
            },
        ...
        <div clas = "box">
            <ul class="form">
                <li class="clearfix"><label><strong>Tên giải </strong>:</label>
                    <div class="filltext">
                        <input type="hidden" style="width:360px" value="<?php echo $footballer["id"]?>" id="id">   
                ...
                <li class="clearfix"><label><strong>Ảnh </strong>:</label>
                    <div class="filltext">
                        ...
                        <input type="hidden" id="avatar_link" value="<?php echo $footballer['avatar']?>"/>    
                        <p id="show_pic">
                              <?php 
                            if($footballer['avatar']!=""){
                                ?>
                                 <img style="width: 100" src="<?php echo Yii::app()->params['urlImages']?>bongda/footballer/<?php echo  date('Y/md', strtotime($footballer['create_date']))?>/<?php echo $footballer['avatar']?>" />
                                <?php
                            }
                            ?>
                        </p>
