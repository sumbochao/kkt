<?php
    class AGamelao extends CActiveRecord
    {
        ...
        public function getDataSearch($from_date,$to_date,$keyword,$type,$status,$orderBy,$compare,$page,$row_per_page){
            $str_sql = "";
            $str_order = " ORDER BY id ";
            switch($orderBy){
                case 1: $str_order = " ORDER BY count_jar";break;
                case 2: $str_order = " ORDER BY count_apk";break;
                case 3: $str_order = " ORDER BY count_ipa";break;
            }
            if(intval($compare) == 1){
                $str_order .= " DESC ";
            }else{
            ...
            $connect = Yii::app()->db;
            $sql = "SELECT count(id) as count FROM c_game_lao WHERE 1 ".$str_sql."";
            $command = $connect->createCommand($sql);
            $data_count = $command->queryRow();
            $max_page = ceil(intval($data_count["count"])/$row_per_page);
            $first = ($page - 1)*$row_per_page;

            $sql = "SELECT * FROM c_game_lao WHERE 1 ".$str_sql." ".$str_order." LIMIT ".$first.",".$row_per_page."";
            //var_dump($sql);
            $command = $connect->createCommand($sql);
        ...
        {
            $sql = "SELECT * FROM c_game_lao WHERE id = ".intval($id);
            $connect = Yii::app()->db;
            $command = $connect->createCommand($sql);
        ...
        public function updateDeleteField($id,$field){
            $sql = "UPDATE c_game_lao SET ".$field."= '' WHERE id = ".intval($id);
            $connect = Yii::app()->db;
            $command = $connect->createCommand($sql);
