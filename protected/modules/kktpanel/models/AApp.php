<?php
    class AApp extends CActiveRecord{
        ...
        public function getCountId(){
            $sql = "SELECT count(id) as value FROM c_app";
            $connect = Yii::app()->db;
        ...
        public function getDataSearch($keyword,$page,$row_per_page){
            ...
            $connect = Yii::app()->db;
            $sql = "SELECT count(id) as count FROM c_app WHERE 1 ".$str_sql."";
            $command = $connect->createCommand($sql);
            $data_count = $command->queryRow();
            $max_page = ceil(intval($data_count["count"])/$row_per_page);
            $first = ($page - 1)*$row_per_page;
            $sql = "SELECT * FROM c_app WHERE 1 ".$str_sql." ".$str_order." LIMIT ".$first.",".$row_per_page."";
            //echo $sql;die;
            $command = $connect->createCommand($sql);
        ...
        public function getDataById($id){
            $sql = "SELECT * FROM c_app WHERE id = ".intval($id);
            $connect = Yii::app()->db;
            $command = $connect->createCommand($sql);
            $data = $command->queryRow();
            return $data;
        ...
        public function updateOrderOtherApp($order_app){
            $sql = "UPDATE c_app SET order_app = order_app+1  WHERE order_app >=".$order_app;
            $connect = Yii::app()->db;
            $command = $connect->createCommand($sql);
        ...
        public function updateOrderApp($id,$order_app){
            $sql = "UPDATE c_app SET order_app = ".$order_app."  WHERE id =".$id;
            $connect = Yii::app()->db;
            $command = $connect->createCommand($sql);
            $data = $command->execute();
            return $data;
