<?php
class ANews extends CActiveRecord{
    ...
    public function getDataByCat($categoryId){
        $sql = "SELECT * FROM c_news WHERE categoryId = ".intval($categoryId);
        $connect = Yii::app()->db;
    ...
    public function getDataSearch($from_date,$to_date,$categoryId,$keyword,$type,$status,$isHot,$page,$row_per_page){
        ...
        $connect = Yii::app()->db;
        $sql = "SELECT count(id) as count FROM c_news WHERE 1 ".$str_sql."";
        $command = $connect->createCommand($sql);
        $data_count = $command->queryRow();
        $max_page = ceil(intval($data_count["count"])/$row_per_page);
        $first = ($page - 1)*$row_per_page;
        $sql = "SELECT * FROM c_news WHERE 1 ".$str_sql." ".$str_order." LIMIT ".$first.",".$row_per_page."";
        $command = $connect->createCommand($sql);
        $data = $command->queryAll();
    ...
    public function getDataById($id){
        $sql = "SELECT * FROM c_news WHERE id = ".intval($id);
        $connect = Yii::app()->db;
        $command = $connect->createCommand($sql);
