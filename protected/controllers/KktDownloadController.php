<?php
    class KktDownloadController extends Controller
    {
            ...
            if($type >0 && $refCode !=""){
                ...
                if($data_user){
                    ...
                    if(empty($extension)){
                        ...
                        {
                            $filename .= "." . $extension;     
                            $url_download=Common::getApp($username, $filename);
            ...
            if(isset($_GET["id"])) {            
                ...
                if(!empty($app) && !empty($username)){                                
                    ...
                    if(empty($extension)){
                        ...
                        $orginal_filename = $filename."." . $extension;
                        if("ipa"!=$extension)
                        {
                            $urlFile=Common::getApp($username,$orginal_filename);
                        }else
                        {
                            $urlFile="itms-services://?action=download-manifest&url=".Common::getApp($username,$filename.".plist");
                        }                              
        ...
        public function actionHotHome()
        {
            ...
            if(!empty($app)){                                
                ...
                if(empty($extension)){
                    ...
                    $filename .= "." . $extension;
                    $urlFile = Common::getImage($filename, "app", "", "");
                    $this->redirect($urlFile);
                }
        ...
        public function actionDownloadGame()
        {
            if(isset($_GET["id"]) && isset($_GET["userId"])) {
                ...
                /* Lấy username */
                $user = Download::getUserInfo($userId);            

                /* Lấy thông tin về game */
                $game = Download::getGame($gameId);

                /* không tìm thấy thông tin client */    
                /* get file extension for device remote */
                $extension = Common::detectFileExtension();
                if(empty($extension)){
                    $this->output["status"] = false;
                    $this->redirect(Url::createUrl("/kktUser/listDevice", array("gameId"=>$gameId)));
                } else {
                    $gameFile = Download::getGameFileOffile($gameId, $extension);    
                    if(empty($gameFile)){
                        echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
                        echo "Chúng tôi chưa hỗ trọ file cho dòng máy này! Vui lòng liên hệ admin";        
                    } else {
                        $this->output["status"] = true;
                        if($game["isOnline"]==0 && $game["isSms"]==0){                                
                            $urlFile = Common::getImage($gameFile["filename"], "game", $gameFile["create_date"]);                                    
                        } else {                                    
                            $urlFile = Common::getImage($gameFile["filename"], "user/" . $user["username"] . "/game");                                    
                        }    
                    }                                
                }                
                ...
                if($this->output["status"] == true){
                    ...
                    $result = Download::updateDownloadGameNew($gameId, $gameFile["id"]);                                        
                    $this->redirect($urlFile);
                }                                            
            } else {
                echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
            ...
            if(isset($_GET["id"]) && isset($_GET["dataId"])) {        
                ...
                if(empty($dataDownload) || empty($video) || (!empty($dataDownload) & !empty($video) && $dataDownload["dataId"]!=$video["id"])){
                    ...
                    if($dataDownload["count"]>=3){
                        ...
                        if(preg_match('/\b(ios)\b/i', $_SERVER['HTTP_USER_AGENT'])||preg_match('/iPhone/si', $_SERVER['HTTP_USER_AGENT'])){
                            ...
                            $upload_dir .= date("Y/md/", $video["create_date"]) . $video["file_mp4"];   
                            $link = "http://kenhkiemtien.com/upload/video/".date("Y/md/", $video["create_date"]) . $video["file_mp4"];
